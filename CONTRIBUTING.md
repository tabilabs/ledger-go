...
Thank you for considering making contributions to Tabi and related repositories!

Tabi uses [Tendermint’s coding repo](https://github.com/tendermint/coding) for overall information on repository
...
1. Either [open](https://github.com/tabilabs/tabi/issues/new/choose)
   or [find](https://github.com/tabilabs/tabi/issues) an issue you have identified and would like to contribute to
   resolving.
2. Participate in thoughtful discussion on that issue.
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the tabi team.
    ...
    8. Please make sure to run `make format` before every commit - the easiest way to do this is having your editor run
       ...
       . There are CI tests built into the Tabi repository and all PR’s will require that these tests pass before
...
some [good first issues](https://github.com/tabilabs/tabi/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Tabi, please create
an [ADR](https://github.com/tabilabs/tabi/blob/main/docs/architecture/README.md) so further discussions can be
...
at `https://github.com/tabilabs/tabi`, the code should never exist
at `$GOPATH/src/github.com/tabilabs/tabi`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/tabilabs/tabi`, and do all the work there.
...
Now `origin` refers to my fork and `upstream` refers to the tabi version. So I can `git push -u origin master` to
update my fork, and make pull requests to tabi from there. Of course, replace `tharsis` with your git handle.
...
with [gogoproto](https://github.com/gogo/protobuf) to generate code for use in Tabi.
...
Tabi uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Tabi repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Tabi utilizes [semantic versioning](https://semver.org/).
...
**Note**: tharsis’s Tabi team currently cuts releases on a need to have basis. We will announce a more
